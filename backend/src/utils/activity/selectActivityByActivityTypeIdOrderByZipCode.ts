...
export async function selectActivityByActivityTypeIdOrderByZipCode(activityTypeId: string, userLat: number, userLng: number) {
        try {
            ...
            const[rows] = await mysqlConnection.execute('SELECT BIN_TO_UUID(activityId) AS activityId, activity.activityCity, CONVERT(activityDescription USING utf8) AS activityDescription, activity.activityGroupName, activity.activityLat, activity.activityLong, activity.activityStreet1, activity.activityStreet2, activity.activityWebsite, activityZipCode FROM activity WHERE haversine(:userLng, :userLat, activity.activityLong, activity.activityLat) <25 ORDER BY haversine(:userLng, :userLat, activityLong, activityLat)', {
                    userLng,
                    userLat
                });
            return rows
